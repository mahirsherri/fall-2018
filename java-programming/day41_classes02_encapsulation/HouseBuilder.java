...
public class HouseBuilder {
	public static void main(String[] args) {
		...
		List<House> housesList3 = Arrays.asList(houses);//fix size arraylist. u cannot add another house
		...
		for(int j = 0; j < housesList.size(); j++) {	
			if(housesList.get(j).year >= 2006 && housesList.get(j).year <= 2016) {
				System.out.println(housesList.get(j).year +" | "+housesList.get(j).address);
			}
		}
